...
const stringsToNumbs = (numbers) => {
  let arrNums = [];
  numbers.forEach(number => {arrNums.push(Number(number))});
  return arrNums
}

console.log(stringsToNumbs(strNums)) //array of digits as numbers
...
const filterStates = (arr) => {
  const unique = arr.map(item => item.weather_state_name).filter((item, index, array) => {return index === array.indexOf(item)})
  return unique;
}

console.log(filterStates(weather));

//find the id of the object in weather that has a min_temp of 15.915
// const findIdMidtemp = (arr, minTemp) => {

const findMinTemp = (arr, minTemp) => {
  return arr.find(item => item.min_temp === minTemp).id;
}

console.log(findMinTemp(weather, 15.915));
